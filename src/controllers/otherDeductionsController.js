...
const updateProcessedStatus = async (req, res) => {
  try {
    ...
    return successResponse(
      res,
      200,
      `Deduction ${processed ? 'marked as paid' : 'marked as unpaid'} successfully`,
      deduction
    );
  } catch (error) {
    console.error("Error updating deduction processed status:", error);
    return errorRresponse(res, 500, "Error updating deduction processed status", error);
  }
};
